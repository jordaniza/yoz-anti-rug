...
contract TokenDistributor is Ownable {
    ...
    constructor(
        ...
        require(
            ...
            "TokenDistributor: zero token address"
        );
        require(
            _sweepReceiver != address(0),
            "TokenDistributor: zero sweep address"
        );
    ...
    function setSweepReciever(address payable _sweepReceiver)
        external
        onlyOwner
    {
        _setSweepReciever(_sweepReceiver);
    ...
    function _setSweepReciever(address payable _sweepReceiver) internal {
        require(
            _sweepReceiver != address(0),
            "TokenDistributor: zero sweep receiver address"
        );
    ...
    function withdraw(uint256 amount) external onlyOwner {
        require(
            ...
            "TokenDistributor: fail transfer token"
        );
    ...
    function claimAndDelegate(
        ...
        require(
            token.delegates(msg.sender) == delegatee,
            "TokenDistributor: delegate failed"
        );
    }
    ...
    function sweep() external {
        ...
        require(
            token.transfer(sweepReceiver, leftovers),
            "TokenDistributor: fail token transfer"
        );
    ...
    function claim() public {
        require(
            block.number >= claimPeriodStart,
            "TokenDistributor: claim not started"
        );
